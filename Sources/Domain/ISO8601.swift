...
public struct ISO8601Fixed: Sendable, Equatable {
    public let year: Int
    ...
    public static func parse(_ string: String) throws -> ISO8601Fixed {
        var bytes = [UInt8]()
    ...
    public static func parse(bytes: [UInt8]) throws -> ISO8601Fixed {
        try bytes.withUnsafeBufferPointer {
            try parse(bytes: $0)
        }
    }

    public static func parse(bytes: UnsafeBufferPointer<UInt8>) throws -> ISO8601Fixed {
